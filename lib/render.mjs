...
// Compact one-message render for `known` items (already-known topics that
// surfaced again). Shown after signal/maybe cards, before the footer.
function renderKnownCompactTG(item) {
  const url = item.url;
  const meta = `${escapeHtml(item.author)} · ${escapeHtml(item.published_relative)}${item.metrics?.likes ? ` · 赞${escapeHtml(String(item.metrics.likes))}` : ''}`;
  const lines = [`<b>🔘 已知 — ${escapeHtml(item.title)}</b>`];
  if (item.verdict_reason) lines.push(`<i>${escapeHtml(item.verdict_reason)}</i>`);
  lines.push(`<a href="${escapeHtml(url)}">${meta}</a>`);
  return lines.join('\n');
}
...
export function renderTelegramMessages(broadcast) {
  ...
  // Compact known items go next — one message per item.
  const knownItems = (broadcast.filtered_summary ?? []).filter((f) => f.verdict === 'known');
  for (const item of knownItems) msgs.push(renderKnownCompactTG(item));
